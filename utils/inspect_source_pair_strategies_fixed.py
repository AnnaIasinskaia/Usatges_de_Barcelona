...
# --------------------------------------------------------------------------------------
# Compatibility shim for loading pickles
# --------------------------------------------------------------------------------------
#
# Pair checkpoints were saved from inspect_source_pair_metrics_checkpointed.py
# while that script was running as __main__. Therefore pickle stored rows as:
#   __main__.PairMetricRow
#
# When we load them inside THIS script, Python again sees current script as __main__,
# but there is no PairMetricRow class here unless we define one.
#
# This dummy class is enough: pickle restores attributes into __dict__, and then
# we immediately convert objects into StrategyRow anyway.
#
class PairMetricRow:
    pass
...
def checkpoint_root(base_path: Path) -> Path:
    return base_path.parent / CHECKPOINT_DIRNAME


def pair_checkpoint_dir(base_path: Path) -> Path:
    return checkpoint_root(base_path) / PAIR_DIRNAME
...
def rank_desc(values: List[float]) -> List[int]:
    """
    Positional ranking with stable order by original index.
    ...
    for r in raw_rows:
        out.append(
            StrategyRow(
                ...
                left_corpus=str(r.left_corpus),
                right_corpus=str(r.right_corpus),
